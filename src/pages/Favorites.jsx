...
export default function Favorites() {
  const { addToCart, favorites, toggleFavorite, trending } = useContext(ShopContext);
  ...
  useEffect(() => {
    if (!favorites.length && !trending.length) {
      setLoading(true);
    } else {
      setLoading(false);
    }
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50">
        <Heart className="w-12 h-12 text-blue-500 animate-pulse" aria-label="Loading icon" />

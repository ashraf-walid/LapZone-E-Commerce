...
import { FaTags, FaMicrochip, FaMemory, FaHdd } from "react-icons/fa";
...
export function LaptopFilters({
    ...
    return (
        <div className="bg-white p-6 rounded-lg shadow-lg transition-all duration-200">
            ...
            {/* Filter Sections */}
            {[
                { name: "Brands", key: "brands", icon: <FaTags /> },
                { name: "Processors", key: "processors", icon: <FaMicrochip /> },
                { name: "RAM", key: "ram", icon: <FaMemory /> },
                { name: "Storage", key: "storage", icon: <FaHdd /> },
            ].map((section) => (
                <FilterSection
                    key={section.key}
                    title={
